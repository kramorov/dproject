<!-- gearbox-catalog/components/GearboxSection.vue -->
<!-- Страница раздела: сетка серий редукторов -->
<template>
  <div class="section-page">
    <Breadcrumbs :items="breadcrumbs" />
    <h1 class="page-title">Редукторы</h1>
    <p class="page-subtitle">
      Выберите серию редуктора
    ...
    <div class="series-grid" v-if="series.length">
      <div
        ...
        :key="s.id"
        class="series-card"
        @click="$emit('selectSeries', s.brandId)"
      >
...
onMounted(async () => {
  try {
    const r = await gearboxApi.list({ limit: 1000 })
    const items = r.data?.data || []

    // Группируем по model_line, запоминаем brandId
    const map = {}
