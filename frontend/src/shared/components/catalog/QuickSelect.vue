...
const breadcrumbs=computed(()=>[{name:'Каталог',url:'#'},{name:props.labels.breadcrumbName||'Каталог',url:'#'},{name:pageTitle.value}])
const detailBreadcrumbs=computed(()=>[{name:'Каталог',url:'#'},{name:props.labels.breadcrumbName||'Каталог',url:'#'},{name:pageTitle.value,url:'#'},{name:product.value?.model_line?.name||''}])
...
async function initSeries(){if(!selectedML.value)return;loaded.value=false;try{const r=await props.api.getQuickSelect(selectedML.value,{});const data=r.data;const groups=[];for(const[key,options]of Object.entries(data.filters||{})){if(!options||!options.length)continue;groups.push({key,label:props.filterLabels[key]||key,options})}filterGroups.value=groups;for(const group of groups){const rule=props.autoSelectRules[group.key];if(!rule)continue;const opts=group.options;if(!opts.length)continue;if(rule==='max'){const sorted=[...opts].sort((a,b)=>(b.value||0)-(a.value||0));activeFilters[group.key]=sorted[0].value??sorted[0].id}else if(rule==='min'){const sorted=[...opts].sort((a,b)=>(a.value||0)-(b.value||0));activeFilters[group.key]=sorted[0].value??sorted[0].id}}await fetchProduct()}catch(e){}loaded.value=true}
