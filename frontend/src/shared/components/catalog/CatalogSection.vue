...
defineEmits(['selectSeries','select','quickselect','engineer','navigate'])
const series = ref([])
...
onMounted(async () => {
  try {
    if (props.api.getSections) {
      // Быстрый путь: выделенный эндпоинт /sections/
      const r = await props.api.getSections()
      series.value = (r.data || []).sort((a, b) => a.name.localeCompare(b.name))
      console.log('[CatalogSection] loaded via getSections:', series.value.length, 'series')
    } else {
      // Фолбэк: группировка всех товаров по model_line
      const r = await props.api.list({ limit: 1000 })
      const items = r.data?.data || []
      console.log('[CatalogSection] loaded', items.length, 'items (fallback)')
      const map = {}
      ...
      console.log('[CatalogSection] series found:', series.value.length)
    }
  } catch (e) {
