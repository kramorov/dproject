<template>
  <div class="client-request-lines-container">
    <div v-if="showComponent">
      ...
      <request-line-edit-component
        v-model:showForm="showRequestLineEditComponent"
        :request_id="request_to_edit_id"
        @data-updated="handleRequestLineEditComponentSubmit"
        @cancel="closeRequestLineEditComponent"
    />
    </div>
  </div>
</template>
...
import ClientRequestLinesComponent from "@/pages/client_request/components/RequestLineEditComponent.vue";
...
const openRequestLineForm = (line = null) => {
  if (request !== null) {
  ...
  showRequestLineEditForm.value = true
}
watch(
  () => props.request_id,
  (newId) => {
