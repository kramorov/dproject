...
def get_linkable_models(equipment_type_ids=None):
    """Модели, к которым можно привязать сертификат (M2M cert_docs)."""
    ...
    models = []
    for cls in all_cls:
        if not hasattr(cls, 'cert_docs'):
            continue
        qs = cls.objects.filter(is_active=True)
        if equipment_type_ids and hasattr(cls, 'equipment_type'):
            qs = qs.filter(equipment_type_id__in=equipment_type_ids)
...
if st.session_state.edit_mode_new or st.session_state.edit_cert_id:
    ...
    with st.form("cert_edit_form"):
        ...
        with col2:
            ...
            # Мультивыбор типов оборудования (M2M)
            default_et_ids = (
...
# ==================== БЛОК УПРАВЛЕНИЯ СВЯЗЯМИ (M2M cert_docs) ====================
if st.session_state.edit_cert_id and not st.session_state.edit_mode_new:
    ...
    if edit_cert:
        ...
        for cls in get_models_with_cert_docs():
            for obj in cls.objects.filter(cert_docs=edit_cert).order_by('name'):
                linked_info.append({
                    'cls': cls,
                    'obj': obj,
                    'display': str(obj),
        ...
        if linked_info:
            for item_link in linked_info:
                lcol1, lcol2 = st.columns([12, 1])
                with lcol1:
                    st.write(f"**{item_link['cls'].__name__}**: {item_link['display']}")
                with lcol2:
                    if st.button("❌", key=f"dellink_{item_link['obj'].id}_{item_link['cls'].__name__}"):
                        item_link['obj'].cert_docs.remove(edit_cert)
        ...
        et_ids = list(edit_cert.equipment_types.values_list('id', flat=True))
        print(f"[CERT_MANAGER] loading linkable models for equipment_type_ids={et_ids}")
        linkable = get_linkable_models(tuple(et_ids))
        print(f"[CERT_MANAGER] linkable count={len(linkable)}")

        # Исключаем уже привязанные
        already_linked = set()
        for cls in get_models_with_cert_docs():
            for obj_id in cls.objects.filter(cert_docs=edit_cert).values_list('id', flat=True):
                already_linked.add((cls.__name__, obj_id))
        linkable = [m for m in linkable if (m['model_name'], m['obj_id']) not in already_linked]
        ...
        if not linkable:
            ...
            if filtered:
                ...
                if st.button("🔗 Привязать", key=f"link_btn_{cert_id}"):
                    ...
                    target_cls = None
                    for cls in get_models_with_cert_docs():
                        if cls.__name__ == obj_meta['model_name']:
                            target_cls = cls
                            break

                    if target_cls:
                        obj = target_cls.objects.filter(id=obj_meta['obj_id']).first()
                        if obj:
                            obj.cert_docs.add(edit_cert)
                            st.success(f"✅ Связь добавлена: {obj_meta['name']}")
                        else:
                            st.error(f"❌ Объект не найден")
                    else:
                        st.error(f"❌ Класс модели не найден: {obj_meta['model_name']}")
    ...
    for item in result['data']:
        ...
        with st.expander(title, expanded=is_selected):
            ...
            with col2:
                ...
                etypes = item.get('equipment_types', [])
                if etypes:
                    names = ', '.join(e['name'] for e in etypes)
                    st.write(f"**Типы оборудования:** {names}")
                else:
                    st.write("**Типы оборудования:** —")
            ...
            # --- Связи сертификата (M2M cert_docs) ---
            cert = CertData.objects.filter(id=item['id']).first()
            linked_info = []
            if cert:
                for cls in get_models_with_cert_docs():
                    for obj in cls.objects.filter(cert_docs=cert).order_by('name')[:20]:
                        linked_info.append({
                            'model_name': cls.__name__,
                            'display': str(obj),
                        })
            if linked_info:
                st.markdown("**🔗 Связи:**")
                for link in linked_info:
                    st.caption(f"• {link['model_name']}: {link['display']}")
            else:
                st.caption("🔗 Нет связей")
