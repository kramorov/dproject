...
#
# ВНИМАНИЕ: ВЕРОЯТНО МЁРТВЫЙ КОД (закомментирован 2026-09-16).
#
#   * Модуль не импортируется ни одним файлом репозитория (проверено grep'ом
#     по всем .py).
#   * Несовместим с текущими моделями: у PneumaticTemperatureOption нет
#     get_for_select, а до недавней правки в файле не было даже импортов
#     typing (NameError: Optional is not defined при импорте).
#   * Реальный подбор пневмопривода живёт в actuator_selector_handler.py →
#     BodyThrustTorqueTable.find_suitable_actuators (models/pa_torque.py);
#     там exd-фильтрация пока отсутствует (см. exd-option.md).
#
# Исходный код оставлен закомментированным ниже для справки. Если модуль
# не понадобится — удалить файл целиком.
...
#     """
#     # Разрешаем совместимые виды Exd ОДИН раз на вызов (не в цикле по моделям).
#     exd_compat_ids = None
#     if selected_params.get('exd_id'):
#         try:
#             from params.exd_models import ExdOption
#             requested = ExdOption.objects.filter(
#                 id=int(selected_params['exd_id'])
#             ).first()
#             exd_compat_ids = (
#                 requested.get_compatible_ids()
#                 if requested else {int(selected_params['exd_id'])}
#             )
...
#
#         # 6. Проверка Exd — семантика «не хуже чем» (см. exd-option.md §1.3):
#         # модель проходит, если хотя бы один приписанный ей вид совместим
#         # с запрошенным видом. Набор совместимых вычислен до цикла
#         # (exd_compat_ids) — без N+1.
#         if selected_params.get('exd_id'):
#             variety_ids = set(item.get('exd_variety_ids') or [])
#             if variety_ids & (exd_compat_ids or set()):
#                 score += 3
