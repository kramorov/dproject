...
class PneumaticActuatorActualActuator(models.Model) :
    """
    Реальный пневмопривод, у которого выбраны опции.
    Свойства заполнены по данным model_line, body, model_line_item
    ведется версионирование для КП. С каждым изменением какого-то параметра номер версии инкрементируется.
    Или нет. Номер версии фиксируется при создании КП или нажатием кнопки "Зафиксировать версию".
    При фиксации версии записывается новая запись - для просмотра изменений.
    Добавить поля ДатаИзменений, ДатаСоздания. Все эти поля сделать потом, при привязке в запросу.
    """
    ...
    model_line = models.ForeignKey(PneumaticActuatorModelLine , blank=True , null=True ,
                              related_name='real_pneumatic_actuator_model_line' ,
                              on_delete=models.SET_NULL ,
                              verbose_name='Серия',
                              help_text='Серия пневмоприводов')
    # Поля для вывода информации:
    # model_line.output_type = models.ForeignKey(ActuatorGearboxOutputType
    # model_line.brand = models.ForeignKey(Brands, blank=True, null=True,
    # model_line.pneumatic_actuator_construction_variety = models.ForeignKey(PneumaticActuatorConstructionVariety , blank=True , null=True ,

    hand_wheel = \
        models.ForeignKey(HandWheelInstalledOption , blank=True , null=True ,
                          related_name='real_pneumatic_actuator_hand_wheel' ,
                          on_delete=models.SET_NULL ,
                          verbose_name='Ручной дублер',
                          help_text=_('Установленный на приводе ручной дублер'))
